  ...
  produto.addEventListener("click", () => {
    if (produtosIdCarrinho[produto.getAttribute("data-code")]) {
      const quantidadeTotal = parseInt(
        produto
          .querySelector(".describe_product")
          .getAttribute("data-quantidade")
      );
      console.log(quantidadeTotal);
      if (
        quantidadeTotal > produtosIdCarrinho[produto.getAttribute("data-code")]
      ) {
...
function popUpCarrinho() {
  ...
  const quantidadeProdutos = Object.values(produtosIdCarrinho).reduce(
    (acc, quant) => acc + quant,
    0
  );
